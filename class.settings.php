...
class GPAISRSettings {
	...
	public function menu() {
		$this->_pagehook = add_options_page( __( 'Google+ Author Free', 'gpaisr' ), __( 'Google+ Author Free', 'gpaisr' ), 'administrator', 'gpaisr', array(
			$this,
			'settings_page'
		) );
	...
	public function settings_page() {
		...
		add_meta_box( 'gpaisr-contentbox-1', __( 'General' ), array(
			$this,
			'metabox_general'
		), $this->_pagehook, 'normal', 'core' );

		// add metabox "howto"
		add_meta_box( 'gpaisr-contentbox-2', __( 'What to do', 'gpaisr' ), array(
			$this,
			'metabox_howto'
		), $this->_pagehook, 'normal', 'low' );

		// add metabox "settings
		add_meta_box( 'gpaisr-contentbox-3', __( 'Settings' ), array(
			$this,
			'metabox_settings'
		), $this->_pagehook, 'normal', 'low' );
		...
		// add metabox "about"
		/*
		add_meta_box( 'gpaisr-contentbox-4', __( 'About', 'gpaisr' ), array(
			$this,
			'metabox_about'
		), $this->_pagehook, 'side', 'default' );
		*/

		add_meta_box( 'gpaisr-contentbox-8', __( 'Important Notice', 'gpaisr' ), array(
			$this,
			'metabox_important_notice'
		), $this->_pagehook, 'side', 'default' );

		// add metabox "share"
		add_meta_box( 'gpaisr-contentbox-5', __( 'Share', 'gpaisr' ), array(
			$this,
			'metabox_share'
		), $this->_pagehook, 'side', 'default' );

		add_meta_box( 'gpaisr-contentbox-7', __( 'Discover', 'gpaisr' ), array(
			$this,
			'metabox_discover'
		), $this->_pagehook, 'side', 'default' );

		// add the metabox "links"
		add_meta_box( 'gpaisr-contentbox-6', __( 'Helpful links', 'gpaisr' ), array(
			$this,
			'metabox_links'
		), $this->_pagehook, 'side', 'default' );
	...
	public function metabox_general( $data ) {
		echo '<p>' . __( 'This plugin will replace your author link in your template with your Google+ Profile link to make sure that your Google+ profile picture will be shown in the search results and/or Google News.', 'gpaisr' ) . '</p>';
	...
	*/
	...
	/**
	 * Shows the user that this is no longer maintained.
	 *
	 * @since  0.8
	 * @access public
	 *
	 * @return void
	 */
	public function metabox_important_notice() {
		echo '<p>';
		_e( 'Please note that Google no longer shows author information in search results. This means that you don\'t necessarily need this plugin. Please also note that due to this fact, this plugin will no longer be maintained.', 'gpaisr' );
		echo '</p>';
	}
	...
	public function metabox_discover( $data ) {
		?>
		<a href="https://wp-buddy.com" target="_blank"><img src="https://wp-buddy.com/secure/google-author-pro.jpg" border="0" alt="" /></a>
	<?php
	}
	...
	function settings() {
		...
		add_settings_field( 'gpaisr_into_header', __( 'Add to the header', 'gpaisr' ), array(
			$this,
			'field_into_header'
		), 'gpaisr', 'gpaisr_section_text' );

		add_settings_field( 'gpaisr_replace_author_link', __( 'Replace author link with the Google Profile URL?', 'gpaisr' ), array(
			$this,
			'field_replacement'
		), 'gpaisr', 'gpaisr_section_text' );

		add_settings_field( 'gpaisr_hide_author_link_in_content', __( 'Show or hide the link in the content-section?', 'gpaisr' ), array(
			$this,
			'field_hide'
		), 'gpaisr', 'gpaisr_section_text' );

		add_settings_field( 'gpaisr_open_in_new_window', __( 'Open the link in a new window?', 'gpaisr' ), array(
			$this,
			'field_new_window'
		), 'gpaisr', 'gpaisr_section_text' );

		add_settings_field( 'gpaisr_rss', __( 'Display in Feed?', 'gpaisr' ), array(
			$this,
			'field_rss'
		), 'gpaisr', 'gpaisr_section_text' );

		add_settings_field( 'gpaisr_link_text', __( 'Link text', 'gpaisr' ), array(
			$this,
			'field_link_text'
		), 'gpaisr', 'gpaisr_section_text' );

		add_settings_field( 'gpaisr_affiliate', '', array(
			$this,
			'field_affiliate'
		), 'gpaisr', 'gpaisr_section_text' );
	...
	public function metabox_links() {
		...
		<ul>
			...
			<li><a href="http://bit.ly/ZaaM8S" target="_blank"><?php echo __( 'Get the full version', 'gpaisr' ); ?></a>
			</li>
			<li>
				<a href="http://bit.ly/UlDG4t" target="_blank"><?php echo __( 'More cool stuff by WPBuddy', 'gpaisr' ); ?></a>
			</li>
		</ul>
	<?php
	}
	...
	/**
	 * Informs the user that Google no longer shows author information in search results.
	 *
	 * @since  0.8
	 * @access public
	 *
	...
	public function admin_notice() {
		$notice_checked = get_option( 'gpaisr_notice_checked', false );

		if ( isset( $_GET['check_notice'] ) ) {
			update_option( 'gpaisr_notice_checked', true );

			return;
		}
		if ( (bool) $notice_checked ) {
			return;
		}
		?>
		<div class="error">
			<p><?php _e( 'Please note that Google no longer shows author information in search results. This means that you don\'t necessarily need the Google Authorship plugin.', 'gpaisr' ); ?>
				<a href="<?php echo admin_url( 'options-general.php?page=gpaisr&check_notice=1' ); ?>"><?php _e( 'Okay. I know. Thanks.', 'gpaisr' ); ?></a>
			</p>
		</div>
	<?php
	}
